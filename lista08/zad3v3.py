...
def gen_wordmaps(simplified_query: str, word_count: int):
    ...
    return words
    # split_generator = i.combinations(range(1, len(simplified_query)), word_count)
    # # add begin of slice and end of slice:
    # split_generator = map(lambda x: (0,) + x + (len(simplified_query) + 1,), split_generator)
    # return ([simplified_query[l:r] for l, r in i.pairwise(splits)] for splits in split_generator)
...
def main():
    find_n_anagrams("Bolesław Prus", 2)
